...
- ```<object>``` 태그
  - ```<object>``` 태그는 오디오, 비디오, 자바 애플릿, PDF 등 다양한 멀티미디어 파일 삽입 시 사용 
~~~
(기본형) 
...
- ```<embed>``` 태그
  - ```<embed>``` 태그는 ```src``` 속성을 사용해 오디오, 비디오, 오브젝트 태그 지원하지 않는 다양한 웹 브라우저에서 멀티미디어 파일 삽입 가능
  - 닫는 태그 없음
...
- ```<audio>```, ```<video>``` 태그
  - 오디오, 비디오를 삽입할 때 사용하는 태그. 사용 구조는 ```<embed>```와 유사 

~~~
(기본형) 
<audio src="파일 경로" [속성="속성값"]></audio>
~~~
  - 관련 속성 
    ...
    - ```<loop>``` : 오디오, 비디오 반복 재생
    - ```<muted>``` : 오디오, 비디오 소리 제거 
    - ```<preload>``` : 페이지 불러올 때 오디오, 비디오 파일 어떻게 로딩할 것인지 지정. 기본값은 ```<preload="auto">```. 이외 ```auto```, ```medadata```, ```none``` 등 값 사용 가능
    - ```<width>, <height>``` : 비디오 플레이어의 너비와 높이 지정. 둘 중 하나 값만 지정하면 나머지는 자동 계산
    - ```<poster="파일 이름">``` : ```<video>``` 태그에서 사용하는 속성으로, 비디오 재생 전까지 화면 표시될 포트서 이미지 지정 
...
- ```<form>``` 태그 
  - 폼을 만드는 가장 기본적인 태그. form 요소를 감싸는 태그를 데이터로 묶어 실제 서버로 전송하는 역할을 하는 태그  
~~~
(기본형)
